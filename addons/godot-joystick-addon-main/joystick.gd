...
func g_in(event: InputEvent) -> void:
	if is_pressed:
		if event is InputEventScreenDrag:
			var g_pos: Vector2 = self.to_global(touch.position+event.position)
			if center_global.distance_to(g_pos+front.size/2)<=max_distance:
				front.global_position = g_pos-front.size/2
			else:
				var angle: float = center_global.angle_to_point(g_pos)
...
func _add_childs():
	...
	var big: Panel = Panel.new()
	big.set("theme_override_styles/panel", style_box)
	self.add_child(big)
	back = big
	var small: Panel = Panel.new()
